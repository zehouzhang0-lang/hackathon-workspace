...
2026-08-29 前端分支合并补充：XLSX 读取在 ZIP 声明、实际流式展开和行列/文本分配前执行资源限制，具体阈值见 [Demo 数据边界](../../../demo/README.md)。损坏、超限、不支持格式分别返回 `invalid_xlsx`、`xlsx_limit_exceeded`、`xlsx_unsupported`；不返回部分事实，不删除原件。合并批次的浏览器与回归结果见项目进度，不把下方旧批次“待验”理解为永久状态。
