...
[四图功能锁定](../../docs/development/demo/WIREFRAME_FUNCTION_LOCK.md)中的显式榨汁杯初始资料、图片接收和 Excel 原件保存已接回当前界面。XLSX 已合入本机已知指标列解析；下面的文件是独立上传样例，不冒充榨汁杯数据，种子只含初始资料，不预设选择、执行或未来反馈。

仅供本地测试，所有数字均为合成。文件最多 6 份，单份 10,000,000 字节，总计 20 MiB。PNG/JPEG/WebP 可接收预览，但没有 OCR；XLSX 读取已知指标列（抖音作品导出、榜单快照、metric 约定表），区间估值不折算单值，整列全 0 而同表其他列有正值时保留采集缺失提示；XLS 旧格式仅保存原件。PDF/HTML/SVG/视频暂不支持。TXT 真实读取，不自动当成结构化经营事实。
