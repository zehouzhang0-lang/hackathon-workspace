...
export function buildDemoAnalysis(state) {
  ...
  try {
    ...
    const priority = {
      ...
      title: priorityActive ? '这轮先看：商品点击后的价值与信任承接' : '当前不确定优先验证环节',
      ...
      hypothesis: priorityActive ? { text: '商品价值、适用边界或购买风险说明不足，可能增加点击后的犹豫；尚未证实。', sourceFactIds: ownerHypothesis ? [ownerHypothesis.id] : [], sourceIds: ownerHypothesis ? ['fact:' + ownerHypothesis.id] : ['input:description'] } : null,
      unknowns: ['价格、商品吸引力或流量质量是否影响加购仍未知。', '投流、退款、投诉与真实顾客疑问资料未提供。']
    };
