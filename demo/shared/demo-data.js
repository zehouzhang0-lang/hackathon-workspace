...
import { ROADSHOW_SHOE_FIXTURE_ID, matchesRoadshowShoeQuestion,
  hasRoadshowShoeFixtureCore, ROADSHOW_ACCOUNT_DIAGNOSIS_FACTS,
  ROADSHOW_SHOE_PLAN_A_STEPS, ROADSHOW_SHOE_PLAN_B_STEPS } from './roadshow-shoe-fixture.js';
...
export function buildDemoAnalysis(state) {
  ...
  try {
    ...
    function shoePath(optionLabel) {
      ...
      const action = (isA ? ROADSHOW_SHOE_PLAN_A_STEPS : ROADSHOW_SHOE_PLAN_B_STEPS).join('；');
      const result = path(title, action, diagnosisKeys);
      result.actionSteps = [...(isA ? ROADSHOW_SHOE_PLAN_A_STEPS : ROADSHOW_SHOE_PLAN_B_STEPS)];
