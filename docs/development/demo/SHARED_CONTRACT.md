...
状态：供本轮Demo审阅；启用后三个页面共同遵守。当前只有契约，**没有共享模块实现**。批准和实现状态以[入口READY表](README.md)为准；变更契约只能由统筹同步通知三页，不能由一页私自改接口。

## 1. 文件归属与依赖

| 写入者 | 允许的应用路径 | 交接路径 |
| --- | --- | --- |
| 统筹 | `demo/shared/**`、`demo/assets/**`、`demo/samples/**`、`demo/tests/**`、`demo/README.md` | 本目录共享文档、根进度与Git |
...
```json
{
  ...
  "round": {
    ...
    "index": 1,
    "inputVersion": 1,
    "clarification": {"limit": 1, "status": "unused", "questionId": null}
  },
  "input": {
    "description": "",
    "focus": null,
...
| 对象 | 约定字段与语义 |
| --- | --- |
| material | `id,name,mime,size,status,sourceKind,blobKey,error`；状态为received/parsed/needs_review/failed；文件名不是可信内容 |
| fact | `id,key,value,availability,unit,subject,window,channel,cohort,source,verification`；availability=known/unknown/not_applicable，未知值为null；0只能来自明确值 |
...
| analysis | `id,roundId,inputVersion,status,mode,summary,paths,limitations`；status=ready/limited/insufficient/stale；mode=demo_fixture/local_limited/real_model，当前默认不实现real_model |
...
| artifact | `id,version,roundId,analysisId,pathId,inputVersion,status,kind,title,body,usage,sourceFactIds`；status=current/stale；每次保存编辑生成新版本 |
...
subscribeSession(listener) // 返回unsubscribe；通知本页及同源其他页面的更新
getMaterialBlob(materialId) // Promise<Blob|null>；null表示无可用原件
```
...
| INPUT_EDIT | `description` | 内容真变才递增inputVersion；输入版本影响规则见下节 |
...
| MATERIAL_RESULT_SET | `materialId,inputVersion,status,facts,error` | 保存实际解析结果；核对材料身份与版本，过期/已删除任务拒绝，失败不删原件 |
| FACT_PATCH | `fact,reason` | 保留来源及更正记录；不能替换为无出处“事实” |
| ORGANIZATION_SET | `focus,facts,constraints,unknowns,inputVersion` | 只接收对应版本的整理结果；不能把分析结论写成事实 |
| QUESTION_SET | `questionId,status` | status=asked/answered/skipped；asked后就耗用，跨页/刷新/同轮改资料不重置 |
...
| ARTIFACT_SAVE | `artifact` | 不覆盖旧版本；旧依据仅可留作历史，不作为当前稿 |
...
- `revision`每次成功事务递增；`inputVersion`只随描述、资料、事实/限制更正及显式新轮次变化。补问状态、浏览、下载等不改inputVersion。
...
首版建议支持PNG/JPEG/WebP、UTF-8 TXT/CSV、约定JSON；最多6份，每份5MiB，总计20MiB。超过范围明确拒绝该文件，保留其他文件。PDF、XLSX、视频不声称支持解析；不接收HTML/SVG可执行素材。禁止eval、导入上传脚本或把未转义文本写innerHTML。

- 图片：真实点击/拖入/粘贴接收、预览、保存、删除；无OCR则状态needs_review，只允许用户主动补一句或按未知继续。
...
- 所有接收与解析只在本机，不发第三方；本轮不做爬虫、银行/收款授权、后台API或监控识别。
